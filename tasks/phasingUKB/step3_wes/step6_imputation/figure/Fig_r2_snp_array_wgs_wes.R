...
j=1
	...
	pdf(paste("Fig_r2.pdf", sep=""), 12, 6, bg="white")
	is_new=FALSE
	par(fig=c(0,0.5,0,1), new=is_new, mar=c(5,5,4,2)+0.1)
	...
	#{
		...
		{
		  file=paste("/home/srubinac/Dropbox/workspace/shapeit5/tasks/phasingUKB/step2_wgs/step8_imputation/figure/data/imputed_1k_rp_",CONFmetP$Met1[r],"_chr20.rsquare.grp.txt.gz", sep="")
		  print(file)
		  if(!file.exists(file))
		  {
			next
...
mets= c("BEAGLE_v5.4","SHAPEIT_v5.0.0")
refs = c("446,470 WES samples")

list_legend=""
list_colours=""
j=1
	is_new=TRUE
	par(fig=c(0.5,1,0,1), new=is_new, mar=c(5,5,4,2)+0.1)
	plot(1,1,type="n", xlim=c(0.00008,50), ylim=c(0.0,1), ylab=expression(paste("Aggregate ", italic("r"),""^"2"*"")), xlab="British Population Minor Allele Frequency (%)", log="x", xaxt='n', las=1)
	mtext(paste0("Target: 1000 samples | Reference:", refs[j]), side=3, line=0)
	axis(1, at=c(0.0001, 0.001, 0.01, 0.1, 1, 10, 50), label=c("0.0001", "0.001","0.01", "0.1", "1", "10", "50"), main=paste0("Imputation performance"))
	abline(v=c(0.0001,0.0002,0.0005,0.001,0.002,0.005,0.01,0.02,0.05,0.1,0.2,0.5,1,2,5,10,20,50), col="lightgrey", lty=3)
	abline(h=seq(0,1,0.05), col="lightgrey", lty=3)
	...
	#{
		...
		for (r in 1:nrow(CONFmetP)) 
		{
		  file=paste("data/imputed_1k_rp_",CONFmetP$Met1[r],"_allchrs.rsquare.grp.txt.gz", sep="")
		 # file=paste("data/test_chr21.rsquare.grp.txt.gz", sep="")
		  print(file)
		  if(!file.exists(file))
		  {
			next

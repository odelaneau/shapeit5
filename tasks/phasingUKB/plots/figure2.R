...
lBIN=c("singleton","2-5","6-10","11-20","21-50","51-100","101-200","201-500","501-1k","1k-2k","2k-5k","5k-10k","10k-20k","20k-50k","50k-100k", "100k-150k")
...
freqSER <- function (prefix, suffix, s) {
...
for (s in nSIZE:nSIZE) {
	...
	BGLser = freqSER(prefix, suffix, s);
	BQ0[s] = BGLser[2]
		
	str1="~/Fuse/PhasingUKB/Phasing/PhasingWGS/step5_runshapeit_phase2/N";
	str2="/benchmark_ukb23352_c20_qc_v1.subset.N";
	prefix=paste(str1, SIZE[s], str2, SIZE[s], sep="")
	suffix=paste("shapeit5.", PAR[1], ".", TYPE[4], ".frequency.switch.txt.gz", sep="")
	SHPser0 = freqSER(prefix, suffix, s);	
	BQ1[s] = SHPser0[2]
	...
	plot(1:(9+s), log10(BGLser), type="n", pch=20, xlab="Minor Allele Count", ylab="Switch Error Rate (%)", col="black", lwd=2, xaxt='n', yaxt='n', main=paste("A. Switch Error Rate\n[N=", SIZE[s], "]", sep=""))
	...
	points(X, (SHPser0 - BGLser) * 100.0 / BGLser, type="o", pch=20, col=COLdiff[2], lwd=2)

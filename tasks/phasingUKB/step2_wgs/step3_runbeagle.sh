...
while read LINE; do
	...
	OUT=benchmark_ukb23352_c20_qc_v1.$REG\.beagle5.3
	TIM=benchmark_ukb23352_c20_qc_v1.$REG\.beagle5.3.time
	
	dx run app-swiss-army-knife -iin="/docker/beagle.25Mar22.4f6.jar" --folder="/Phasing/PhasingWGS/step3_runbeagle/" -icmd="/usr/bin/time -vo $TIM java -Xmx256G -jar $BGL gt=$VCF map=$MAP out=$OUT nthreads=32 chrom=chr20 && bcftools index -f $OUT\.vcf.gz --threads 8" --tag benchWGS --tag beagle5.3 --tag chr20 --instance-type mem3_ssd1_v2_x32 --priority normal --name benchWGS_beagle5.3_$REG -y
